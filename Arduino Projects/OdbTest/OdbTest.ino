...
void setupBlueToothConnection()
{
  ...
  Serial.print("setup bt connection");
  ...
  sendATCommand("BIND=000d,18,000001");    //send BIND=??, bind HC-05 to OBD bluetooth address
  sendATCommand("INIT");                   //send INIT, cant connect without this cmd 
  delay(1000); 
  sendATCommand("PAIR=000d,18,000001,20"); //send PAIR, pair with OBD address
  delay(1000);  
  sendATCommand("LINK=000d,18,000001");    //send LINK, link with OBD address
...
void sendATCommand(char *command)
{
  ...
  if (!(bt_error_flag)){                                  //if no bluetooth connection error
    ...
    if (retries>=BT_CMD_RETRIES) {                        //if bluetooth retries reached
      ...
      Serial.print("bt error flag is true");
